...
namespace Server.Controllers
{
    ...
    public class ProductsExpensesController : ControllerBase
    {
        ...
        [Authorize]
        [HttpGet("prod-base-info/{infoId}/user/{userId}")]
        public async Task<ProductsExpenseDTO> GetByProductBaseInfoId(int infoId, string userId)
        {
            var gId = Guid.Parse(userId);

            var expense = await productsExpensesService.GetProductsExpenseByProductBaseInfoIdAsync(infoId, gId);
            return expense;
        }
