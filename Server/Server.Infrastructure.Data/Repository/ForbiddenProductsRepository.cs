...
{
    public class ForbiddenProductsRepository : BaseRepository<int, ForbiddenProduct>
    {
        public ForbiddenProductsRepository(ApplicationDbContext ctx) : base(ctx) { }

        public async override Task<ForbiddenProduct> GetAsync(int id)
        {
            return await table.FirstOrDefaultAsync(p => p.Id == id);
        }

        public async override Task UpdateAsync(ForbiddenProduct value)
        {
